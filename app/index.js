...
var callIdentifier = require('./routes/callIdentifier.js');//Xirsys API module
var app = express()
    .use(cors())
    .use(bodyParser.json())//json parser
    .use(bodyParser.urlencoded({ extended: true }))//urlencoded parser
    .use('/videochat', express.static(path.join(__dirname, 'public')))//path to examples
    .use("/webrtc",webrtc(xirsys))//watch API calls
	.use("/videochat",callIdentifier());//watch API calls
