...
function WebRtc(xirsys) {
    ...
    router.use('/:method/', function(req, res, next){
        if(req.params.method != null && req.params.method == '_path'){console.log("********ENTER**********AB");
    ...
    router.use(function (req, res) {
		...
		const buf = Buffer.alloc(50, xirsys.info.ident+":"+xirsys.info.secret).toString("base64").substring(0,64);
            var options = {
                method: req.method,
                host: xirsys.gateway,
                path: req.url,
